...
- For $B$, any value equal to red is incompatible with $A \neq B$, so remove red from $D_B$. New domain: $D_B = \{green, blue\}$.

- For $C$, the same reasoning applies. New domain: $D_C = \{green, blue\}$.
...
- a CSP $(X,D,C)$,
- an objective function $f: \text{complete assignments} \to \mathbb{R}$ to be minimized or maximized.
...
- objective: $cost(s) = \text{number of violated constraints in state } s$, or a weighted version.

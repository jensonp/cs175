...
### Scope and mechanism lock

DQN is not minimizing one fixed stationary supervised objective with immutable labels. Its targets are bootstrapped, policy-dependent, and periodically refreshed as learning changes value estimates.

Target-network update mechanism should be read concretely: practical DQN uses either hard periodic copies ($w^- \leftarrow w$ every $C$ updates) or slow moving-average updates of $w^-$.

Replay and stale transitions matter for optimization behavior, but they are not the definition of off-policy. Off-policy remains a policy-mismatch statement between behavior data generation and target-policy structure.
...
So even a low prediction loss on the sampled training distribution does not certify that the representation is Markov-sufficient for control.
...
DQN is off-policy because replayed transitions are typically generated by exploratory behavior, while the target continuation uses greedy maximization at the next state.
...
Neither device eliminates approximation, bootstrapping, or off-policy learning. Each one instead reduces how violently those ingredients interact over short training windows.

A common hidden assumption is that the max operator always provides a neutral control target. Under noisy approximate values, the $\max$ can introduce positive overestimation bias because it preferentially selects upward errors. This is one reason Double DQN-style decoupling appears in later methods.
...
### Short note: maximization bias and why Double DQN was proposed

The max in the DQN target can overestimate continuation value when action-value estimates are noisy, because the same noisy estimates are used to both select and evaluate the maximizing action. Double DQN addresses this by decoupling selection and evaluation roles. This refinement does not change the chapter's central DQN story, but it is the natural next step once max-operator bias is understood.
...
Replay and target networks are stabilizers, not theorem-level convergence guarantees in nonlinear approximation settings.
...
No gradient is propagated through the target-network branch, through next-state target values, or through the maximizing action-selection step.
...
So aliasing is not only a function-approximation nuisance; it is evidence that the chosen representation may fail the Markov-sufficiency role needed for exact value recursion.

...
If the objective integrand has explicit parameter dependence (for example $f_\theta(\tau)$), an additional term $\mathbb E_{p_\theta}[\nabla_\theta f_\theta(\tau)]$ must be carried; the pure score-function form alone is no longer complete.
...
Equivalent condition: after conditioning on $S_t=s$, the baseline must be fixed with respect to the action draw $A_t\sim\pi_\theta(\cdot\mid s)$.
...
Typical bias sources worth naming separately are critic approximation error, bootstrap target error, truncated estimators (including GAE-like truncation choices), stale or off-policy data used by the critic, and surrogate-objective modifications in downstream methods.
...
Clipping is a practical surrogate mechanism for conservative updates. In common deep function-approximation settings it should not be read as an exact trust-region method or as a general lower-bound theorem on true policy performance.
...
For continuous-action policies, sums become integrals and one works with differential entropy under the policy density.
...
In continuous-action SAC implementations, policy expectations are taken under densities, so integral forms and differential-entropy conventions must be handled explicitly.

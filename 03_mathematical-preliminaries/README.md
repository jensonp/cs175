...
Suppose the weather variable $Y$ can be either sunny or rainy. Let the reward variable $X$ represent the payoff from taking an outdoor action. The reward distribution is:

- If $Y=\text{sunny}$, then $X=2$ with probability $0.8$ and $X=-1$ with probability $0.2$.
- If $Y=\text{rainy}$, then $X=2$ with probability $0.1$ and $X=-1$ with probability $0.9$.

We want to compute $\mathbb{E}[X \mid Y=\text{sunny}]$ and $\mathbb{E}[X \mid Y=\text{rainy}]$.
...
- The first post-decision reward contributes $4$.
- The second contributes $-1$.
- The third contributes $1.5$.
- The fourth contributes $0$.
...
There are no further terms because $R_4$ is the reward generated by the final action at time $3=T-1$.

...
Suppose the current state-action pair is $(s,a)$, and the environment has the following one-step law.

- With probability $0.3$, the next state is $s_1$ and the reward is $5$.
- With probability $0.7$, the next state is $s_2$ and the reward is $-1$.
...
- **Definitions** include $V^\pi$, $Q^\pi$, and $A^\pi$.
- **Exact identities** include the return recursion, the relation $V^\pi(s)=\sum_a \pi(a\mid s)Q^\pi(s,a)$, and the Bellman expectation equations.
- **Approximations** have not yet entered. They arrive later when exact expectations are replaced by sampled targets or function approximation.
...
What should the reader see in this example? The exact stopping point of evaluation changed, but the underlying logic did not. The invariant pattern is the loop:

- assess the current policy,
- prefer better actions using that assessment,
- repeat under the new policy.
...
A chapter on value functions and Bellman equations introduces several pairs of ideas that are close enough in form to be regularly confused. If those confusions survive, later material becomes unstable. This section gathers the most common ones and corrects them explicitly.
